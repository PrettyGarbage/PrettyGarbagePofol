...
using Debugg = UnityEngine.Debug;
...
public class MoPubOSCommands {
    ...
    public static bool RunExe(StringBuilder stdout, string exe, string args, string dir, string[] vars)
    {
        ...
        try {
            ...
            proc.WaitForExit();
            if (proc.ExitCode != 0) {
                Debugg.LogErrorFormat("Error exit while running '{0} {1}': {2}\n{3}", exe, args, proc.ExitCode, stderr);
                return false;
            }
        } catch (Exception e) {
            Debugg.LogErrorFormat("Exception while running '{0} {1}': {2}", exe, args, e);
            return false;
        }
        return true;
    }
    ...
    internal static bool Cp(string source, string dest)
    {
        try {
            ...
            Debugg.LogErrorFormat("Exception while copying file '{0}' to '{1}': {2}", source, dest, e);
            return false;
        }
    }
    ...
    public static bool Mv(string source, string dest)
    {
        if (source.EndsWith("/*")) {
            ...
            if (!dest.EndsWith("/")) // Make sure dest is treated as a directory.
                dest += "/";
            Debugg.LogFormat("Moving directory contents from '{0}' to '{1}'", source, dest);
        ...
        if (!File.Exists(source) && !Directory.Exists(source)) {
            Debugg.LogWarningFormat("Source doesn't exist: {0}", source);
            return false;
        }
        ...
        if (File.Exists(dest) || Directory.Exists(dest)) {
            Debugg.LogWarningFormat("Ignoring, destination already exists: {0}", dest);
            return false;
        }

        Debugg.LogFormat("Moving '{0}' to '{1}'", source, dest);
        try {
            ...
            Debugg.LogErrorFormat("Exception while moving '{0}' to '{1}': {2}", source, dest, e);
            return false;
        }
    }
    ...
    {
        try {
            Debugg.Log("Removing file: " + path);
            File.Delete(path);
            path += ".meta";
            if (File.Exists(path)) File.Delete(path);
            return true;
        } catch (Exception e) {
            Debugg.LogErrorFormat("Exception while deleting file '{0}': {1}", path, e);
            return false;
        }
    }
    ...
    {
        Debugg.Log("Removing directory: " + path);
        if (!Directory.Exists(path))
        {
            Debugg.LogWarningFormat("Ignoring, directory not found: {0}", path);
            return false;
        }
        try {
            ...
            Debugg.LogErrorFormat("Exception while deleting '{0}': {1}", path, e);
            return false;
        }
        return true;
    }
    ...
    public static bool Sed(string file, string regex, string repl)
    {
        try {
            ...
            Debugg.LogErrorFormat("Exception while rewriting file {0}: {1}", file, e);
            return false;
        }
    }
    ...
    public static IEnumerable<string> GetFileSystemEntries(string dir, string pattern = null)
    {
        try {
            ...
            Debugg.LogErrorFormat("Exception while finding files in dir {0}: {1}", dir, e);
    ...
    public static bool Rsync(string srcDir, string destDir, params string[] names)
    {
        ...
        if (names.Length == 0) {
            Debugg.LogErrorFormat("No matching files found in dir {0}", srcDir);
            return false;
        }
