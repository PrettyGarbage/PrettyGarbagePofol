...
#if ADDRESSABLE_SUPPORT
            if(InstantiateType == InstantiateType.InstantiateByAddressable) RegisterModalsByPrefab = RegisterModalsByAddressable.Select(reference => reference.LoadAssetAsync().WaitForCompletion()).ToList();
#endif
...
#pragma warning restore 4014
        ...
        public void OnAfterDeserialize()
        {
            ...
            if (InstantiateType == InstantiateType.InstantiateByAddressable)
            {
                ...
                RegisterModalsByAddressable = RegisterModalsByAddressable.Distinct().ToList();
            }
            else
            {
                RegisterModalsByAddressable.Clear();
                RegisterModalsByPrefab = RegisterModalsByPrefab.GroupBy(x => x.GetType()).Select(x => x.FirstOrDefault()).ToList();
            }
#endif
        }
