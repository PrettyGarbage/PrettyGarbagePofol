...
namespace Mine.Code.Framework.Manager.UINavigator.Runtime.Page
{
    public sealed class PageContainer : UIContainer<PageContainer>, IHasHistory, ISerializationCallbackReceiver
    {
...
#if ADDRESSABLE_SUPPORT
            if(InstantiateType == InstantiateType.InstantiateByAddressable) RegisterPagesByPrefab = RegisterPagesByAddressable.Select(reference => reference.LoadAssetAsync().WaitForCompletion()).ToList();
#endif
            ...
            Action<Page> onPostInitialize = null)
        {
            ...
            Util.Debug.Debug.LogError($"Page not found : {nextPageName}");
            return null;
        }
            ...
            Action<Page> onPostInitialize = null)
        {
            ...
            Util.Debug.Debug.LogError($"Page not found : {nextPageType.Name}");
            return null;
        }
        ...
        public void OnAfterDeserialize()
        {
            ...
            if (InstantiateType == InstantiateType.InstantiateByAddressable)
            {
                ...
                RegisterPagesByAddressable = RegisterPagesByAddressable.Distinct().ToList();
            }
            else
            {
                RegisterPagesByAddressable.Clear();
                RegisterPagesByPrefab = RegisterPagesByPrefab.GroupBy(x => x.GetType()).Select(x => x.FirstOrDefault()).ToList();
            }
#endif
        }
