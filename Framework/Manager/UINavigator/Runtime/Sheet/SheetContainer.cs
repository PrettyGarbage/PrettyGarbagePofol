...
#if ADDRESSABLE_SUPPORT
            if (InstantiateType == InstantiateType.InstantiateByAddressable) RegisterSheetsByPrefab = RegisterSheetsByAddressable.Select(reference => reference.LoadAssetAsync().WaitForCompletion()).ToList();
#endif
