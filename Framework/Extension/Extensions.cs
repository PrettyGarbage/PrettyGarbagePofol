...
namespace Mine.Code.Framework.Extension
{
    ...
    public static class Extensions
    {
        ...
        public static IEnumerable<TSource> ForEach<TSource>(this IEnumerable<TSource> collection, Action<TSource> action, bool isReverse = false)
        {
            if (isReverse)
            {
        ...
        {
            Util.Debug.Debug.Log(string.Join(", ", collection.Select(x => x.ToString())));
        }
        ...
        // UniRx를 이용하여 게임 오브젝트 파괴와 함께 코루틴을 종료하는 기능을 확장 메소드로 정의합니다.
        public static void AddTo(this Coroutine coroutine, GameObject addTo, MonoBehaviour mono) => addTo.OnDestroyAsObservable().Subscribe(_ => mono.StopCoroutine(coroutine));
        public static void AddTo(this Coroutine coroutine, MonoBehaviour mono) => mono.gameObject.OnDestroyAsObservable().Subscribe(_ => mono.StopCoroutine(coroutine));
        ...
        public static void Print<T>(this T obj) => Util.Debug.Debug.Log(obj.ToString());
        public static void Print<T>(this T obj, string prefix) => Util.Debug.Debug.Log($"{prefix} : {obj}");
        ...
        //라이프타임 스코프에 등록하고 등록 이후에 콜백 함수를 실행 시킨다.
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        ...
        #region AsyncOperation Extensions

        public static void SafetyHandler<T>(this AsyncOperationHandle<T> handle, Action<AsyncOperationHandle<T>> handler)
        {
